...
const VendorData = require("../models/vendor/vendorModel");//vendor schema
const UserModel = require("../models/user/userDocument")// user  documentation schema
...
// ----------------------------------------------Vendor registration data starts------------------------------//
...
exports.SignupController = async (req, res) => { 
  try {
      // console.log(req.body)
      const { firstName, lastName, email, phoneNumber, password } = req.body;
      const isExist = await VendorData.findOne({ email: email })
      if (!isExist) {
          const hashedPassword = await bcrypt.hash(password, 10);
          const user = new VendorData({ firstName, lastName, email, phoneNumber, password:hashedPassword });
          user.save();
          return res.status(201).send({ success: true, message: "User Registered Successfully"});
      }
      else {
...
exports.loginController = async (req, res) => {
  try {
      ...
      // }
      // get user from db
      const user = await VendorData.findOne({ email: email });
      // console.log(user)
      // check user
      if (!user) {
          return res.status(404).send({ success: false, message: "User Not Found" });
...
exports.otpController = async (req, res) => {
  ...
  try {
    ...
    const user = await VendorData.findOne({ email });
    if (user) {
      // console.log("user found");
      const otp = generateOtp();
      // console.log(otp);
...
exports.NewPasswordController = async (req, res) => {
  try {
      ...
      const result = await VendorData.findByIdAndUpdate({ _id: user._id }, {
          password:hashedPassword
      })
      console.log(result)
      return res
...
exports.userDocumentController = async (req, res) => {
  try {
      ...
      if (!isExist) {
          const user = new UserModel({ name, email, number, address });
          await user.save();
          return res.status(201).send({ success: true, message: "User Registered Successfully" });
      } else {
          return res.status(200).send({ success: false, message: "User Already Registered, Please Login" });
